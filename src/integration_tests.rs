...
mod test {
    ...
    async fn websocket_writer(port: u16, header: &[u8], file: Vec<u8>) {
    ...
    async fn websocket_reader(port: u16, header: &[u8]) -> Vec<u8> {
        ...
        let mut output: Vec<u8> = Default::default();
        while let Some(Ok(msg)) = futures::StreamExt::next(&mut client).await {
            assert!(msg.is_binary());
            output.extend_from_slice(msg.as_payload());
        }
        output
    }
    ...
    #[cfg_attr(not(feature = "local_db_tests"), ignore)]
    #[tokio::test(flavor = "multi_thread")]
    async fn integration_test_server_websocket_one_writer_one_reader() {
        // Game ID in db: 2000
        ...
        let sent_replay = get_file("example");
        let replay_writer = websocket_writer(port_info.websocket.unwrap(), b"P/2001/foo\0", sent_replay.clone());
        let replay_reader = websocket_reader(port_info.websocket.unwrap(), b"G/2001/foo\0");
