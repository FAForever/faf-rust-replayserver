...
pub trait MergeStrategy {
    ...
    fn replay_new_delayed_data(&mut self, id: u64);
...
pub async fn track_replay(s: &RefCell<impl MergeStrategy>, r: ReplayRef) {
    let token = s.borrow_mut().replay_added(r.clone());

    /* Avoid holding ref across an await */
    let wait_for_finish = r.borrow().wait(StreamPosition::FINISHED(0));
