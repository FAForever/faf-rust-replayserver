...
use super::{PositionTracker, StreamPosition};
    ...
    async fn advance_some(t: Rc<RefCell<PositionTracker>>, mut c: Receiver<()>) {
    ...
    async fn await_test(t: Rc<RefCell<PositionTracker>>, when: StreamPosition, c: Sender<()>) {
    ...
    async fn simple_test() {
        let tracker = Rc::new(RefCell::new(PositionTracker::new()));
        let (send, recv) = channel::<()>(1);
        join! {
            await_test(tracker.clone(), StreamPosition::DATA(0), send.clone()),
            await_test(tracker.clone(), StreamPosition::DATA(5), send.clone()),
            await_test(tracker.clone(), StreamPosition::DATA(10), send.clone()),
            await_test(tracker.clone(), StreamPosition::FINISHED(10), send.clone()),
