...
use crate::{database::queries::GameTeams, database::queries::{ModVersions, Queries}, error::SaveError};
...
mod test {
    ...
    fn serialized_header_looks_as_expected() {
        // This was generated BY this test, but was also checked by hand. It looks like it should.
        let expected2 = r#"{"complete":true,"featured_mod":"faf","featured_mod_versions":{"faf":3718},"game_end":10000,"game_type":"unknown","host":"Sheeo","launched_at":9000,"mapname":"adaptive_syrtis_major.v0001","num_players":10,"recorder":"MazorNoob","state":"PLAYING","teams":{"2":["FtxCommando","Lyuker","Paralon","Chosen"],"1":["Mephi","Sheeo","MazorNoob","Downlord"]},"title":"100k+","uid":9999999}"#;
        /* A bit paranoid, but let's make sure. */
        let mut mod_versions = HashMap::new();
        mod_versions.insert("faf".into(), 3718);
        let mut teams = HashMap::new();
        teams.insert("1".into(), vec![String::from("Mephi"), "Sheeo".into(), "MazorNoob".into(), "Downlord".into()]);
        teams.insert("2".into(), vec![String::from("FtxCommando"), "Lyuker".into(), "Paralon".into(), "Chosen".into()]);
