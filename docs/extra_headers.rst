...
randomly close websockets. This section describes the extension to the protocol
that were added in order to support this.
...
data before the initial connection header. Where the initial connection header
looks like so:

``G/12423353/name\0``

Each piece of that extra data is a string delimited with a null byte. The server
reads these strings **and the initial connection header**, then responds with
other null-delimited strings. The client should send all the strings plus the
connection header before expecting a response.
